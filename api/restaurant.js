...
router.route("/")
    ...
    .post((req, res) => {
        ...
        const restaurant = restaurants.find(r => r.name === req.body.name);

        if (restaurant) {
            return res.status(409).json({message: "The restaurant with the given name is already exist!"});
        }
        else {
            const card = {
...
router.route("/:name")
    .put((req, res) => {
        ...
        if (!restaurant) {
            return res.status(404).json({message: "The restaurant with the given name not found!"});
        }
        ...
        // Save the updated restaurants array back to data.json
        fs.writeFile(
            path.join(__dirname, "..", "data", "data.json"),
            JSON.stringify(restaurants, null, 2),
            (err) => {
